...
The opening questions to the client are themselves a challenge. Industrial design for rackmount work often drops you into a domain you don't know — telecom, server hardware, industrial automation — and the brief assumes context the designer hasn't yet built. You can't walk in unprepared. Talk to engineers and architects in your own company first; that gives you enough baseline that the conversation becomes useful. Pre-interview research often answers most of the basics, freeing the actual interview to focus on the unknowns. If you're already in the interview with no time for any of that, fall back on two openings: "describe a day in your life," or "explain what you do as if I were a child." And keep reading. Eventually you become the expert.
...
The patterns translate across into automotive surface finishing, which is worth knowing because high-end industrial work borrows the same techniques. Interior plastics are typically ABS+PC with soft-touch coating; visible aluminium gets a hairline-brushed finish. Alcantara — synthetic suede — shows up too. Its reputation has been damaged by low-quality Chinese knockoffs and amateur upholstery jobs, but for an armrest-style surface it's genuinely good. Aluminium goes into a design only when it beats both plastic and steel on the relevant metrics: it's lighter than steel but more expensive; obviously more expensive than plastic but more durable; better at conducting current than steel. The first all-aluminium car body was built in 1899; today aluminium bodies are mostly a premium feature, with midrange cars using aluminium for hood and rims but keeping the roof and bonnet on plain steel. Heavier transports that need both strength and low weight use aluminium too. For automotive, the relevant series are 5000, 6000, 7000. If the brief calls for heat treatment, 5000 is out.

Plastics translate across just as transparently. Every familiar object is a small material stack. A plastic bottle is a label of polypropylene, a body of PET (polyethylene terephthalate), and a cap that's typically PP plus HDPE (low-density polyethylene). Bottles for carbonated drinks or cosmetics need impact-resistant bimodal HDPE like SABIC HDPE CC254, which is clear. ABS turns up. And the less safe PVC. Once you've categorised the products around you, picking the right plastic for an industrial enclosure stops feeling abstract.
...
For small portable devices that need on-board file storage, the question of which filesystem to use (exFAT? sdcardfs?) is usually the wrong one to ask. On Android-based hardware the right answer is Google's Storage Access Framework — files become documents, and the access pattern stops being filesystem-specific. That frees the hardware design from being coupled to a particular storage stack.
