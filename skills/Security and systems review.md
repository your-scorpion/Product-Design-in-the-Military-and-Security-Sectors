...
A maturity signal means something different depending on how far along the design actually is. Don't judge an early concept for lacking what a production system needs, and don't excuse a production system for lacking what an early concept could reasonably skip.

Before assigning a maturity signal to any category, decide first whether that category is even stage-relevant yet:

- **Not yet relevant, and unaddressed** — mark it *not yet relevant*, not Missing. A concept-stage sketch with no monitoring plan hasn't failed at monitoring; monitoring isn't due yet.
- **Not yet relevant, but addressed anyway** — a stated, credible deferral ("not building this yet; revisit once the pilot has real usage data") is a good sign, not a gap. Grade the deferral itself, not the absence of the thing deferred.
- **Genuinely stage-relevant** — grade normally. Here, silence really is the failure the Missing signal describes.

**Minimum evidence, by stage:**

| Stage | What actually needs to exist |
|---|---|
| Concept | A one-sentence threat model — what's protected, from whom. A rough sense of who the actual users or operators are. An honest read on which constraints (physical, regulatory, connectivity) are even relevant yet. |
| Design doc | Every applicable review category gets at least a first-pass answer. Non-goals are stated explicitly. A validation approach is described, even if not yet run. Any compliance-relevant claim names the actual standard or control it maps to. |
| Prototype | A working baseline exists, and the more sophisticated approach is measured against it, not just described as better. First-pass error analysis. The access-control or physical-constraint boundary has been tested at least once, not only described as an intention. |
| Pilot / shadow mode | A review protocol with an actual reviewer count and cadence, not "we'll review some of it." A stated, falsifiable safety threshold. A fallback path that's actually been triggered and observed once, not only documented. |
| Production | Every monitoring metric has a named owner and review cadence. Incident response and rollback have been rehearsed, not only written down. Independent validation — by someone outside the build team — has happened at least once. |
| Maintenance | A stated re-verification cadence for anything that can go stale: metadata, clearance levels, cost ratios, model or vendor drift checks. A plan for what happens as the system outgrows the assumptions it launched with. |

A category graded harshly against the wrong stage is a worse mistake than a category left ungraded — when in doubt about which stage a design is actually at, ask, rather than defaulting to the more demanding standard.
